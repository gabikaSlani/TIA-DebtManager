...
const getGroups = (request, response) => {
  ...
  if (isNaN(id)) {
    ...
    pool.query(' SELECT gr.id, gr.name, debt FROM ' +
      ' (SELECT g.name,g.id, ' +
      '     ((SELECT coalesce(sum(debts.amount), 0) from debts JOIN items ON debts.item_id=items.id AND items.group_id=g.id AND debts.settled=false AND receiver_id=$1)-' +
      '      (SELECT coalesce(sum(debts.amount), 0) from debts JOIN items ON debts.item_id=items.id AND items.group_id=g.id AND debts.settled=false AND payer_id=$1)' +
      '     ) as debt ' +
      ' FROM user_in_group as ug ' +
      '  JOIN groups as g ' +
      '  ON g.id=ug.group_id AND ug.user_id=$1 ' +
      ' ) as gr ' +
      ' ORDER BY abs(debt) DESC;',
      [id], (error, results) => {
        if (error) {
          throw error;
        }
...
const addGroup = (request, response) => {
  const {id, groupInfo} = request.body;
  const name = groupInfo.name;
  tx(async client => {
    const {rows} = await client.query('INSERT INTO groups (name, date, creator_id) VALUES ($1, NOW() ,$2) RETURNING id', [name, id]);
    const groupId = rows[0].id;
    console.log('groupId:' + groupId);
    groupInfo.chosenFriends.forEach(async friend => {
      await client.query('INSERT INTO user_in_group (user_id, group_id) VALUES ($1, $2)', [friend.value, groupId]);
    });
    await client.query('INSERT INTO user_in_group (user_id, group_id) VALUES ($1, $2)', [id, groupId]);
    console.log('all friends added to group');
  })
...
const getGroupById = (request, response) => {
  ...
  if (isNaN(id)) {
    ...
    console.log('get group with id: ', id);
...
const getGroupMembers = (request, response) => {
  const id = parseInt(request.params.id);
  if (isNaN(id)) {
    console.log('id is NaN');
    throw error;
  } else {
    console.log('get group members: ', id);
    pool.query('SELECT users.id, users.login ' +
      'FROM groups JOIN user_in_group as ug ON ug.group_id=groups.id JOIN users ON ug.user_id=users.id ' +
      'WHERE groups.id=$1 ' +
      'ORDER BY users.login', [id], (error, results) => {
      if (error) {
        throw error;
      }
      response.status(200).json(results.rows);
...
const getItemsInGroup = (request, response) => {
  const userId = parseInt(request.params.userId);
  ...
  if (isNaN(userId) || isNaN(groupId)) {
    ...
    console.log('get group items: ', userId, groupId);
    pool.query('SELECT items.id, items.name, items.amount, u.login as creator, u.id as creatorId, d.amount as debt, d.settled ' +
      'FROM items ' +
      'JOIN groups as g on items.group_id = g.id AND g.id=$2' +
      'JOIN users as u on items.creator_id = u.id ' +
      'JOIN debts as d on items.id = d.item_id AND (d.receiver_id = $1 OR d.payer_id = $1) ' +
